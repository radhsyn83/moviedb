...
class GetGenresUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Resource<Page<List<Genre>>>> = flow {
        try {
            emit(Resource.Loading<Page<List<Genre>>>())
            val res = repository.genres().toGenres()
            emit(Resource.Success<Page<List<Genre>>>(res))
        } catch(e: HttpException) {
            emit(Resource.Error<Page<List<Genre>>>(e.localizedMessage ?: "An unexpected error occured"))
        } catch(e: IOException) {
            emit(Resource.Error<Page<List<Genre>>>("Couldn't reach server. Check your internet connection."))
        }
    }
}